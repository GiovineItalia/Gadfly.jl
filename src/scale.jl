...
function color_discrete_hue(; levels=nothing, order=nothing,
                            preserve_order=true)
    DiscreteColorScale(
        h -> convert(Vector{OpaqueColor},
             distinguishable_colors(h, OpaqueColor[LCHab(70, 60, 240)],
...
function color_discrete_manual(colors...; levels=nothing, order=nothing)
    cs = OpaqueColor[color(c) for c in colors]
    function f(n)
...
function apply_scale(scale::ContinuousColorScale,
    ...
    for (aes, data) in zip(aess, datas)
        ...
        color_key_colors = Dict{OpaqueColor, Float64}()
        color_key_labels = Dict{OpaqueColor, String}()
