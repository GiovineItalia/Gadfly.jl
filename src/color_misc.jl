...
function lab_rainbow(l, c, h0, n)
    OpaqueColor[LCHab(l, c, h0 + 360.0 * (i - 1) / n) for i in 1:n]
end

function luv_rainbow(l, c, h0, n)
    OpaqueColor[LCHuv(l, c, h0 + 360.0 * (i - 1) / n) for i in 1:n]
end

# Helpful for Experimenting
function plot_color_scale{T <: OpaqueColor}(colors::Vector{T})
    println(colors)
...
function lab_gradient(cs::OpaqueColor...)
    if length(cs) < 2
